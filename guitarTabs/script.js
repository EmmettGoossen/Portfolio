...
function createTab(note,fret,beats,prev){
  return {
    ...
    beatTime: prev.beats*song.Signature[1]+prev.beatTime, //how long until the beat is ready to be drawn
    ...
    play() {
      let time = this.beatTime*(1/pace.value); //bigger pace = faster = smaller beat time
      //we need to be able to pause while keeping the beat time correct, so we set up the wait object first
      if(this.ready[0]){
        this.draw();
        this.x -= this.vx;
        return;
